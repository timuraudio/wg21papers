...
However, a major limitation of the initial feature set in C++26 is that \tcode{pre} and \tcode{post} cannot be placed on a \emph{virtual} function; doing so renders the program ill-formed. This limitation exists due to disagreements in WG21 regarding the desired behaviour of \tcode{pre} and \tcode{post} on virtual functions that could not be resolved in the C++26 timeframe.
...
However, in the remainder of this paper, we generally use the terms \emph{correct call} or \emph{correct implementation} of a function to mean {correct with respect to its contract assertions}, i.e., without violating them. 
% TD: TODO: Don't do this. Don't corrupt the meaning of "Correct", invent a different term
Consistently, we use the term \emph{contract violation} to mean a violation of a contract assertion (regardless of whether that violation was detected by a runtime check), and not a violation of the plain-language contract.
% TD: TODO: Do we need a new term for violating a contract covered by an assertion? Or can we just say that some are not detected, or that they *can* be detected but not always are?

% TD: TODO: Somewhere also introduce canonical and non-canonical, and group use cases accordingly

% TD: TODO: Search below: "Enforce DbC" is not a use case. Maybe frame these things as use cases, not properties? Then the wording and impl experience also kinda goes away. Or maybe this is a "sanity" use case (see categories in DbC)
 
\subsection{Direct and indirect calls}

% TD: TODO: rewrite this: a virtual function call through a non-reference non-pointer is a direct call and should be treated as such.
