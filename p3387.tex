...
Crucially, the callee of \tcode{pre} and \tcode{post} on the coroutine declaration is \emph{not} the coroutine body --- the function body that the user wrote --- and \tcode{pre} and \tcode{post} on the coroutine declaration do not assert anything directly related to that body. The coroutine body may resume long after the postconditions that apply to the declaration are checked, may run on another thread, and never directly sees the object being returned from the function.
