...
Such implicit \tcode{const}-ness of the parameter type being triggered by odr-use in a postcondition assertion can even lead to paradoxes. Consider the following example where \tcode{i} is not odr-used if it is \tcode{const}, but odr-used if it is not:
\begin{codeblock}
void f(int i)
  post([&]() {
