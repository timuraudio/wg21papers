...
We believe that the Lakos Rule should remain a design principle for the C++ Standard Library. There are important engineering reasons why the Lakos Rule is still valid and useful today; abandoning it would inflict avoidable damage to the C++ language . To argue this case, we published two papers in the May 2023 mailing, \cite{P2831R0} and \cite{P2861R0}, to be presented at the June 2023 meeting in Varna.
...
\item \textbf{Programmatic recovery.} There are applications (e.g., in gaming, desktop publishing software, host-plugin systems) where throwing an exception on precondition failure and recovering programmatically is vastly preferable to terminating. However, a \tcode{noexcept} barrier turns any such throw into unconditional \tcode{std::terminate}. If the Lakos Rule is abandoned, such applications are permanently prevented from using any conforming Standard Library implementation. 
...
Violations of the Lakos Rule are not random, but principled (at least, before C++26) and almost all of them are clearly localised in clusters:

\begin{itemize}
\item Functions whose body cannot throw by the very nature of the operation, even if called out-of-contract: \tcode{std::atomic}, deallocation functions, and compiler intrinsics with no runtime effect such as \tcode{std::launder} and \tcode{std::start_lifetime_as}.
\item Getter/observer methods for smart pointers, \tcode{std::optional},  \tcode{std::expected}, \tcode{std::span}, and \tcode{std::mdspan} (whose preconditions tend to be hardened as of C++26).
\item A few Lakos Rule violations in facilities newly introduced in C++26, such as \tcode{std::hive} and \tcode{std::saturating_div}, showing divergence in the absence of a policy.
\end{itemize}

Conditionally-\tcode{noexcept} functions with narrow contracts are few. All of them fall into a narrow, well-defined idiom that has been explicitly designed with the Lakos Rule in mind: \tcode{swap}, \mbox{\tcode{iter_swap}}, \tcode{iter_move}, and the forwarding \tcode{operator()(Args...)} in \tcode{std::move_only_function} and \tcode{std::copyable_function}, where conditional \tcode{noexcept} tracks the throwing behaviour of a template parameter while the precondition stays narrow. These functions carve out precise exceptions to the Lakos Rule where algorithmically necessary, exactly as intended by the original \tcode{noexcept} policy.

Apart from the above clusters and a small handful of isolated cases, the C++ Standard Library consistently adheres to the Lakos Rule. The most popular and widely used C++ Standard Library facilities adhere to it \emph{without any exceptions}: all containers introduced before C++26 (\tcode{std::vector}, \tcode{std::array}, associative containers), all algorithms introduced before C++26, as well as \tcode{std::variant}, \tcode{std::pair}, \tcode{std::tuple},  \tcode{std::mutex}, \tcode{std::chrono}, etc. Other popular facilities such as \tcode{std::thread} are entirely wide-contract so the question does not arise.

Our analysis demonstrates that the Lakos Rule is the norm in the C++ Standard Library today. Further, the Standard actively uses the ``\emph{Throws:} Nothing'' specification idiom for narrow-contract functions that follow the Lakos Rule. Therefore, reinstating the Lakos Rule as official LEWG policy is the only choice consistent with the status quo in the C++ Standard Library specification.
...
\section*{Disclaimer}
The quantitative survey of Lakos Rule adherence across the entire C++ Standard Library has been performed with the help of an LLM. The exact prompts used for this survey and their full output are available from the authors upon request.
