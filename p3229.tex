...
An expression \tcode{E} that evaluates to an erroneous value is thus essentially equivalent to the following function:
...
% TD: Josh, in the following sentence, my brain cannot parse any of the stuff after the first comma. Can you please break up the sentence and simplify it so it's clearer what you're trying to say?
...
\cite{P2973R0} ``Erroneous behaviour for missing return from assignment'' proposes that flowing off the end of a copy or move assignment operator results in erroneous behaviour and is erroneously equivalent to a return with operand \tcode{*this}. With this proposal, the equivalent wording that achieves the same effect would be that ... %TD: Josh, I'm struggling to formulate this, can you help? I can't think of any way to formulate this in terms of a boolean predicate.

\cite{P3232R1} ``User-defined erroneous behaviour'' proposes a new Standard Library function \tcode{std::erroneous()} which invokes erroneous behaviour when called and has no effect otherwise. With this proposal, a call to \tcode{std::erroneous()} becomes essentially obsolete as it is exactly equivalent to \mbox{\tcode{contract_assert(false)}}. 

%TD: Josh, are you aware of any other such papers? I checked wg21.link/index.html and there don't seem to be any
