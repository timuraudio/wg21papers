...
The first step towards a unified design is to extend the concept of \emph{contract assertion} in \cite{P2900R13} and to distinguish between \emph{explicit} contract assertions and \emph{implicit} contract assertions. Explicit contract assertions are those that \cite{P2900R13} proposes. They are authored by the \emph{user} by adding function contract specifiers --- \tcode{pre(...)} and \tcode{post(...)} --- and assertion statements --- \mbox{\tcode{contract_assert(...)}} --- to the program. By contrast, implicit contract assertions are generated and injected into the program by the \emph{implementation}.

TODO 

% - erroneous behaviour is entirely replaced by the notion of implicit contract assertion
% - mention it can be applied in the same way to other defects that EB was considered for (signed integer overflow etc)
% - mention that this is not the only place where implicit contract assertions play a role, cite Herb P3081 and potentially many more when we implement P3100 more fully
...
% TD: Josh, can you write up this part? I could try but I think you have a much deeper understanding of how this is supposed to work or why it is necessary
