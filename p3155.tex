...
\end{itemize}
The first and fourth clusters are well-motivated divergences from the policy established via \cite{P0884R0}; the second and third are in line with the policy; the last one indicates recent divergence in the absence of an oficial policy.

Conditionally-\tcode{noexcept} functions with narrow contracts are few, and are localised in the following clusters:
\begin{itemize}
\item Swap and move operations: \tcode{std::swap}, member \tcode{swap}, \mbox{\tcode{iter_swap}}, \tcode{iter_move}, move-assign operators;
\item Forwarding \tcode{operator()(Args...)} in \tcode{std::move_only_function} and \tcode{std::copyable_function}.
\end{itemize}
