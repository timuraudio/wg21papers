...
Finally, UB in the Templates category {temp.inst.inf.recursion\}) is unique in that it does not actually represent runtime UB, but instead is a purely compile-time issue, and therefore checking for it at runtime makes no sense.
...
For all other cases of runtime-checkable UB (63 cases in total), we cannot imagine any meaningful fallback behaviour. % TIMUR TODO: mention that termination is always a solution but that's not fallback behaviour!
...
\{temp.inst.inf.recursion\}. This case should be specified as ill-formed instead of UB. % TIMUR TODO: Core issue?
