...
\end{codeblock}
Given this function declaration, if we could drop the \tcode{const} on the defining declaration of \tcode{f}, we could write a dummy implementation of \tcode{f} as follows:
\begin{codeblock}
...
Note that choosing Option~1 would leave the door open to adopting Option~2 or Option~3 at some point in the future, Option~2 could only be relaxed to Option~3 but not strengthened to Option~1, and Option~3 could not be relaxed to either of the two other options.
...
\begin{adjustwidth}{0.5cm}{0.5cm}
If the predicate of a postcondition assertion of a function odr-uses ([basic.def.odr]) a
non-reference parameter of that function, that parameter shall be declared \tcode{const} \added{on all declarations of that function and any function overriding it, }and shall not have array or function type.
\begin{note}
This requirement applies even to declarations
that do not specify the \grammarterm{postcondition-specifier}. Arrays and functions are still usable when declared with the equivalent pointer types ([dcl.fct]).
\end{note}
