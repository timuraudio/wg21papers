...
%TODO: I think we need a revision history, otherwise we will frustrate a lot of readers.
...
A slightly less aggressive approach is to deprecate a language construct and then, in a later standard, remove it.  Historically, however, the committee has been very wary of acting on such deprecations if there is a belief that the code broken by a removal will be frequent, especially if the downside o fthe bad construct is not generally catastrophic.   In general, even breaking incorrect code such that it would fail to compile can make the cost of migrating a large codebase to a new C++ standard unbearably high if the problematic code is ubiquitous and its negative impact often benign.  
...
\subsection{Adding replacement behaviour}
\label{specifyeb}
