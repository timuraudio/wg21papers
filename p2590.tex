...
\emph{Preconditions:} [\tcode{p}, \tcode{(char*)p + sizeof(T)}) denotes a region of allocated storage that is a subset of the region of storage reachable through ([basic.compound]) \tcode{p}.

\emph{Effects:} Implicitly creates objects within the denoted region as follows: an object \tcode{A} of type \tcode{T}, whose address is \tcode{p}, and objects nested within \tcode{A}. The object representation of \tcode{A} is the contents of the storage prior to the call to \tcode{start_lifetime_as}. The value of each created object \tcode{O} of trivially-copyable type \tcode{U} is determined in the same manner as for a call to \tcode{bit_cast<U>(E)} ([bit.cast]), where \tcode{E} is an lvalue of type \tcode{U} denoting \tcode{O}, except that the storage is not accessed. The value of any other created object is unspecified. \begin{note}The unspecified value may be indeterminate.\end{note}
...
\item \textbf{R2}, 2022-06-17: Moved wording for \emph{reachable through} from [ptr.launder] to [basic.compound]; minor wording fixes following CWG review.
\end{itemize}
