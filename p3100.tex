...
We manually inspected all occurrences of the words ``undefined'' and ``assume'' in revision \cite{N5008} of the C++ working paper.
%TD: Should we repeat this exercise for the more current revision N5014? Is there a way to do this more efficiently than having to search for all occurrences again?
...
Overall, as shown in Figure~\ref{piechart_locally}, only 19 cases of UB (23.75\% of all cases) are unconditionally locally diagnosable at run time, while 58 cases of UB (72.5\%) are not; these cases require instrumentation of the code in order to be diagnosed at run time. We will discuss these requirements in more detail in Section~\ref{cost_nonlocal}. In addition, 4 cases of UB (5\%) are not checkable at all in the general case, even with additional instrumentation.
...
\begin{figure}[t!]
    ...
    \begin{tikzpicture}
        \pie[text=legend, 
            ...
            {19/Checkable locally, 6/Checkable locally in some cases, 51/Checking requires instrumentation, 4/Checking not possible in general}
    \end{tikzpicture}
...
Overall, as shown in Figure~\ref{piechart_replacement}, we can define meaningful replacement behaviour for only 17 cases of UB (21.25\% of all cases). Out of these 17 cases, for 3 cases this is only possible when the operation in question produces a value of built-in type. Unconditional replacement behaviour exists for only 14 cases of UB (17.5\% of all cases). In all of these cases, the replacement behaviour consists of erroneous behaviour; in most cases, removing the UB in this manner introduces significant runtime cost.
% TD: "replacement behaviour consists of erroneous behaviour" sounds awkward
...
As we saw in Section~\ref{diagnose}, the vast majority of UB (77 cases out of 80, or 96.25\% of all cases) can in principle be diagnosed by inserting and performing a suitable runtime check. The check verifies the conditions necessary for the operation in question to have well-defined behaviour at run time. It also acts as a guard against UB: if the check fails, the program can be terminated, thus preventing the undefined behaviour from occurring.
...
% TD: Explain that some of these runtime checks are theoretically possible but really infeasible - race conditions
...
\pagebreak % MANUAL PAGEBREAK INSERTED HERE
