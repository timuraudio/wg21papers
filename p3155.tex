...
Violations of the Lakos Rule are not random, but principled (at least, before C++26). Almost all of them are clearly localised in the following clusters:
\begin{itemize}
\item Functions whose body cannot throw by the very nature of the operation, even if called out-of-contract: \tcode{std::atomic} and \tcode{std::atomic_ref}, compiler intrinsics with no runtime effect such as \tcode{std::launder} and \tcode{std::start_lifetime_as};
\item Accessors for smart pointers, \tcode{std::optional},  \tcode{std::expected}, \tcode{std::span}, and \tcode{std::mdspan};
\item Deallocation functions;
