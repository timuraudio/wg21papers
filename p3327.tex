...
\end{codeblock}
The example above shows assignment to a function pointer variable. Again, the question is whether the assignment should compile, and if it does, which assertions should be evaluated in which order when the call through \tcode{fptr} is made. (We can construct an analogous test case for initialising a function pointer variable, or passing a function address into a function taking a function pointer as a parameter.)
...
If all involved function pointers have the same contract assertion sequence, the situation seems obvious. This code should ``just work'' and the contract assertion sequence should be checked on calls through either pointer. But what if the two involved pointers have different contract assertions? Consider, as a first example, assigning the value of one function pointer to another function pointer:
\begin{codeblock}
...
typedef int (*nonnegative_ftpr_t)(int i) pre (i >= 0);
nonnegative_fptr_t get_fptr() { return f; }
...
\end{codeblock}
Again, the pointer used to make the call does not have any contract assertions --- in fact, it is now a prvalue that \emph{cannot} have its own contract assertions (see \ref{syntax_expr}). It  seems even less clear what the behaviour should be in this case. On the one hand, there is no caller-facing contract directly at the call site; on the other hand, if \tcode{get_fptr()} has a contract assertion sequence specified on its return type, a user might find it surprising if that contract is not checked in the call above.

% TODO: we might have to drill down a bit more here and distinguish between contracts that are (at least theoretically) visible at the call site and contracts that are not. This section needs more work. We can move some parts from the "property of the declaration" section up here. This would also get rid of some duplication.
...
void test(int i, bool b) {
  (b ? get_positive_fptr() : get_negative_fptr())(i);  // which precondition is checked?
}
\end{codeblock}
...
If we allow the callee-facing contract (the contract on the function that ends up being called) and the caller-facing contract (that of the pointer being called) to be different, this option is still a possible conservative choice; if we do not, it is seems to be the only option that makes any sense. However, we again need to make sure that it is impossible to SFINAE on whether two entities have the same contract assertion sequence. 
...
\end{codeblock}
Here, for the call at (1) the caller-facing contract is that of the pointer to member function (currently required to be empty by \cite{P2900R8} --- this restriction would be relaxed if we allow \tcode{pre} and \tcode{post} on function pointers), and the callee-facing contract is still that of  \tcode{Z::f}; the latter contract is the only one that will be checked. The contracts of \tcode{X::f} and \tcode{Y::f} are both intermediate contracts that will be ignored.
...
\begin{codeblock}
void test(int i, bool b) {
  (b ? get_positive_fptr() : get_negative_fptr())(i);
}
\end{codeblock}
where the pointers returned by \tcode{get_positive_fptr()} and \tcode{get_negative_fptr()} have different contracts.
...
When we invoke a function, we do not invoke a declaration; we invoke an expression. If that expression is an \emph{id-expression} that directly names a function or a function pointer whose declaration is visible, and \emph{that} declaration has \tcode{pre} and \tcode{post} attached to it, the backchannel information is readily available. However, as we already saw in \ref{intermediate}, there are many situations where this is not the case. Consider:
\begin{codeblock}
struct C {
...
\begin{codeblock}
typedef int (*nonnegative_ftpr_t)(int i) pre (i >= 0);  // (1)
