...
\begin{itemize}
\item \textbf{R6}, 2026-05-12: Added full wording for all cases of UB; rebased on \cite{P3596R1}; added history and polls section
...
% TD: TODO: CONTINUE FROM HERE: pie chart shows 3 cases as "Not checkable in some cases", you need to find the other 2 and then revise the chart. Then, start over at the top and fix all numbers.
...
Overall, as shown in Figure~\ref{piechart_locally}, only 17 cases of UB (21.25\% of all cases) are unconditionally locally diagnosable at run time, while 8 cases of UB (10\%) are only locally diagnosable under some conditions, and 52 cases of UB (65\%) are not locally diagnosable at all, requiring instrumentation of the code in order to be diagnosed at run time. We will discuss these requirements in more detail in Section~\ref{cost_nonlocal}. In addition, 3 cases of UB (3.75\%) are not diagnosable at all in the general case, even with additional instrumentation.
...
\begin{figure}[t!]
    ...
    \begin{tikzpicture}
        \pie[text=legend, 
            ...
            {17/Checkable locally, 8/Checkable locally in some cases, 52/Checking requires instrumentation, 3/Checking not possible in general}
    \end{tikzpicture}
