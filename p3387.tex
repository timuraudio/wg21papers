...
The only exception to the above is that in a coroutine, function parameters are effectively always non-\tcode{const} objects, and may be moved from by the implementation, even if they are declared \tcode{const}, which is not true for a non-coroutine function. Therefore, the question arises whether \tcode{post} on a coroutine should see the original parameter objects (which may have moved-from values), the parameter copies created by the implementation for the coroutine (which the original values will have been moved to), or whether something else should happen. In the non-coroutine wrapper \tcode{g}, this question cannot arise, as only the original parameter objects are visible to \tcode{g}, and the choice of copying or moving parameters that might be used in \tcode{post} is up to the user and not the compiler.

In this paper, we discuss this particular issue in more detail and propose a solution that is consistent with the fundamental design principles of both coroutines and Contracts.
...
Our wording strategy to achieve the above (see Section \ref{wording}) is to not change or add anything to the specification of the point of evaluation of \tcode{pre} and \tcode{post} in \cite{P2900R8}, but instead to clarify that all operations that are specific to the compiler-generated ramp function, in particular creating a copy of the parameters and allocating the coroutine frame, is considered part of the function body. With this wording clarification, the sequencing of evaluating \tcode{pre} and \tcode{post} relative to these coroutine-specific operations directly follows from the existing wording in \cite{P2900R8}.

\subsection{Referring to parameters in \tcode{post}}
\label{paramsinpost}

There is one implementation detail of coroutines that is unobservable caller-side today, but would be made observable if we allow \tcode{post} to apply to a coroutine declaration, making a proposal to allow function contract assertions on coroutines non-trivial.

When the ramp function is invoked, after initialising the function parameters (and checking the precondition assertions, if we allow them), a copy of each parameter is created in the coroutine frame, and the value of each parameter is moved into that copy ([\href{https://eel.is/c++draft/dcl.fct.def.coroutine#13}{dcl.fct.def.coroutine.13}]), potentially leaving the original parameter object in a moved-from state. This is necessary to make the parameters accessible in the coroutine frame, whose lifetime can extend far beyond the ramp function returning.

Notably, this moving-from a parameter happens even if that parameter is declared \tcode{const}: the ramp function removes the top-level \tcode{const} from the parameter, and then modifies that parameter by using it to move-construct the copy. By contrast, in a non-coroutine function, such an attempt to move from a parameter declared \tcode{const} (e.g., by using a \tcode{const_cast}) would be undefined behaviour as per \href{https://eel.is/c++draft/dcl.type.cv#4.sentence-1}{[dcl.type.cv.4]}. 

Effectively, in a coroutine, a parameter object is never actually \tcode{const}, even if declared \tcode{const} in the function declaration ([\href{https://eel.is/c++draft/dcl.fct.def.coroutine#note-2}{dcl.fct.def.coroutine.13 Note 2}]). This specification may seem strange, but does not cause issues in C++ today because the moved-from parameter values in a coroutine are not exposed or made accessible to the user in any way, consistent with the design principle that coroutine-ness is an implementation detail. However, if we allowed the predicate of \tcode{post} on a coroutine to refer to parameters, we would expose this implementation detail to the caller, as \tcode{post} is evaluated after the move-from has happened, and would see potentially moved-from values, even though such a parameter is declared \tcode{const}. This might be unexpected to users not familiar with how coroutines are implemented under the hood.

Going back to the design principle that coroutine-ness is an implementation detail, we are faced with the following situation. Given a function declaration where the return type is awaitable, such a function could be implemented either as a coroutine or as a non-coroutine; the difference is currently unobservable to the caller of the function. However, if we add a \tcode{post} to this function declaration, and let its predicate refer to a parameter, which requires that this parameter be \tcode{const} and assumes that it has not been modified in the body of the function, such a function could be implemented correctly only as a non-coroutine. Any Contracts proposal needs to handle this situation in a consistent and user-friendly way.

\subsection{The solution space}
\label{solutions}

There are different possible ways in which the moved-from parameter question for \tcode{post} on coroutines could be addressed. In order from least to most consistent with the principle that coroutine-ness should be an implementation detail, these are:
\begin{enumerate}
...
Of course, in order to make the proposed Contracts facility as user-friendly as possible, optimising for the design principles that coroutine-ness is an implementation detail is not the only concern, and there are other design goals to consider. We discuss these design goals in the following section.
...
Note that Solution 3 does not violate this design goal because it does not introduce a new inconsistency. One of the necessary limitations of \tcode{post}, compared to \tcode{pre}, is that it is ill-formed to refer to non-\tcode{const} non-reference parameters in the predicate of \tcode{post}, as such parameters may be modified in the function body, making it impossible for the caller to reason about the postcondition (see \cite{P2900R8}, Section 3.4.4). A non-reference parameter is not \tcode{const} if it is not declared \tcode{const} in the function declaration. In addition, a non-reference parameter is never \tcode{const} if the function is a coroutine (see ([\href{https://eel.is/c++draft/dcl.fct.def.coroutine#note-2}{dcl.fct.def.coroutine.13 Note 2}]), even if it \emph{is} declared \tcode{const} in the function declaration. Therefore, implementing a function as a coroutine is just another way of making a non-reference parameter non-\tcode{const}.
...
However, in order to be able to enable caller-side checking of \tcode{pre} and \tcode{post}, the predicate cannot depend on anything that is not accessible caller-side and only known callee-side. For coroutines in particular, it means that \tcode{post} cannot refer to the coroutine frame copies of parameter objects. Solution 4 is therefore not compatible with caller-side checking of \tcode{post}, even though it does allow caller-side checking of \tcode{pre}.

On the other hand, we are aware of fewer use cases for caller-side checking of \tcode{post} than there are for \tcode{pre}, so the former seems less important to support than the latter, and might not even be implementable on some platforms.\footnote{Notably, the Microsoft ABI performs argument destruction callee-side, not caller-side. Since postcondition assertions are specified in \cite{P2900R8} to be evaluated before argument destruction happens, they cannot be checked caller-side on this platform without an ABI break.}
...
\begin{adjustwidth}{0.5cm}{0.5cm}
If the predicate of a postcondition assertion of a function odr-uses ([basic.def.odr]) a non-reference parameter of that function, \added{that function shall not be a coroutine ([dcl.fct.def.\allowbreak coroutine]) and }that parameter shall be declared \tcode{const} and shall not have array or function type.
\end{adjustwidth}
