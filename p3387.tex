...
From some perspectives, the coroutine has a second, extended, \emph{coroutine interface}. This interface covers the actions of the coroutine body which might, for example, repeatedly yield values via \tcode{co_yield}, suspend activity pending some condition via \tcode{co_await}, or return a final value via \tcode{co_return}.  The behaviour of this extended interface is governed both by the user-authored body and the promise and awaiter types that implement the C++ customisation points required. The extended interface is unknown and opaque to the caller, which might no longer exist when parts of the coroutine body execute.
...
Crucially, the callee to which \tcode{pre} and \tcode{post} on the coroutine declaration apply is \emph{not} the coroutine body --- the function body that the user wrote --- and \tcode{pre} and \tcode{post} on the coroutine declaration do not assert anything directly related to that body. The coroutine body may resume long after the postconditions that apply to the declaration are checked, may run on another thread, and never directly sees the object being returned from the ramp function.
%IDS I wonder if we are not being a bit repetitive here and maybe we could extract any new information from this para and add it to the preceding one?  Plus the pre and post do not apply to the body of any function [although, perhaps to side-effects of the body visible at the containing scope], right? (so we do not really need to say that?)
...
%IDS Does it actually alter correctness, or 'merely' incur a performance penalty?  [I accept that the copy might be observable by a CTOR written to do so, but that also applies to a move CTOR]   Perhaps end-users might be quite happy to accept that performance penalty as a trade-off to being about to make const-qualified  parms that apply to all cases?  I am wondering if this is not actually the least distance from the two key criteria [non-visibility of coroutine-ness and contract PD].  I guess the 'against' point here is that the same process would have to be used for all coroutines independent of the application of a post - since we cannot allow the program to change by such an addition - which does violate the "only pay for what you use".
...
%IDS  - I would prefer that we are completely clear that "prior to any coroutine-specific" includes the possibly user-authored coroutine state frame allocator, since that has access to the original parms.
