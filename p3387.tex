...
This paper incorporates the material from \cite{P3251R0}. It also builds on \cite{P2957R1} and has significant overlap with it, but approaches the problem with a design goal-driven approach and seeks to provide a more exhaustive analysis of the available solution space. 
...
\subsection{Contracts on the coroutine function interface}

It follows directly from the above design principle that when \tcode{pre} and \tcode{post} are applied to the declaration of a coroutine, then the function they effectively apply to is the so-called \emph{ramp function} creating the coroutine frame, whose function body is defined by ([\href{https://eel.is/c++draft/dcl.fct.def.coroutine#5}{dcl.fct.def.coroutine.5}]), and \emph{not} to the coroutine body, i.e. the function body that the user wrote. Therefore, \tcode{pre} on a coroutine is an assertion on the parameters passed into the ramp function, \emph{before} the coroutine frame is constructed, and the state of the program at that point; \tcode{post} on a coroutine is an assertion on the object that the ramp function returns to the caller at the point of first suspension, and the state of the program at that point.

This might seem confusing if one is not familiar with how coroutines work. According to one of the design principles of the Contracts MVP (\cite{P2900R8}, Section 3.1, Design Principle 11), \tcode{pre} and \tcode{post} sit at the boundary of the interface and the implementation of a function, and serve both sides equally. However, for coroutines, there is a certain disconnect between the function interface (i.e. the coroutine declaration) and the implementation (i.e. the coroutine body that the user wrote) that does not exist for non-coroutine functions: the coroutine body may resume long after the postconditions that apply to the declaration are checked, or finish long before then\footnote{Note that a coroutine frame can be destroyed on initial suspend, which would be before \tcode{post} is checked.}, and may even run on another thread. It is important to realise that this disconnect is a fundamental property of coroutines in C++ and is not changed or made any worse by adding Contracts.
...
% TODO: Wording might need more work; the last wording blob above might not be correct or sufficient to specify the point of evaluation correctly.

\section{Further considerations}

\subsection{Contracts on the extended coroutine interface}

In this paper, we focus on the function contract assertions of a coroutine declaration, i.e. the function interface of a coroutine. However, a coroutine also has an extended interface, which goes beyond the initial function call and the returned state at the point of first suspension. This extended interface offers the ability to yield multiple values via \tcode{co_yield}, to await these values via \tcode{co_await}, as well as to return a final value via \tcode{co_return}. It would be desirable for the coroutine author to be able to specify precondition and postcondition assertions on these values and the state of the program at these points in some way. 

Since it is not visible on the function declaration whether a function is a coroutine, contract assertions on this extended interface cannot be specified on the function declaration, at least not without modifying the basic design of coroutines (which we do not contemplate in this paper). 

We could certainly consider a novel feature to specify such contract assertions. Such a feature would be a pure extension of the proposal in this paper with no impact on the existing design of \tcode{P2900R8} or the modifications proposed here. Such a feature is also not necessary to achieve consistency between coroutines and non-coroutine functions with regards to \tcode{pre} and \tcode{post}. Finally, we might not even need a new feature to specify contracts assertions on the extended coroutine interface.

Note that \cite{P2900R8} already allows the usage of \tcode{contract_assert} inside a coroutine body, which can be used to express contract assertions on the suspend and resume points inside a coroutine. Alternatively, we can have the coroutine's awaitable and promise type bear the responsibility of carrying the postcondition contract for the values being returned. The latter is a general solution that does not even require the function to be a coroutine, but instead specifies what the yielded and/or returned value are to be constrained to.

The coroutine's returned type plus its associated promise type have the concrete functions that would, in a theoretical setup, be the ones to which the contract assertions would naturally be applied. The promise type's \tcode{yield_value} and \tcode{return_value} functions receive the yielded or returned values and are in an ideal position to bear an assertion as a precondition over them, where the future type's retrieval functions (such as \tcode{await_resume}) carry the same assertion as postcondition. These places are already eligible to receive a precondition and/or postcondition assertions, and the ability to do this requires no change to \cite{P2900R8}.

In effect, this technique places a precondition/postcondition pair on the awaitable as functioning as a channel for the value it carries, with the contract on one hand being asserted on the generating side, and on the other side being made available on the receiving side. This both ensures that if the contract is violated we are made aware at the generating side, and that the contract being valid is available to the compiler on the receiving side for optimisation (if applicable). We can make the assertion to be used on the yielded or returned value(s) a part of the returned type, for example by making it a custom returned type with the contracts asserted, or as template parameters used in a general type to represent the contracts that is carried into the pre- and postcondition. 

\subsection{Coroutine invariants}

Coroutines may have invariants that hold during the execution of the coroutine. As a simple example, a generator could assert that the values generated are strictly increasing. Such an invariant os, similar to the above concept of returned and yielded values having a postcondition that is passed into the return type as a template parameter, a contract that is applied by the promise type on the suspend and resume points. It can similarly be passed into the promise type as a template parameter, made available to the promise type in some other way, or instead asserted with \tcode{contract_assert} directly in the coroutine body.
