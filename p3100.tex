...
Each individual case of UB in our list has a stable identifier. Separately from this paper, \cite{P3596R1} proposes to add a UB and IFNDR annex to the C++ Standard; the proposed enumeration of core language UB in that proposed annex, and the stable identifiers used therein, are in sync with \hyperref[appendix]{Appendix~A} of this paper. In this paper, we place those identifiers between \{curly braces\} to visually distinguish them from the C++ working paper's clause identifiers, which we place between [square brackets]; note that \cite{P3596R1} uses [square brackets] for both.
