...
% TIMUR TODO: There is some duplication of this paragraph with the above, and the "composed of..." part should probably be mentioned further above.
Now, if the user instead chooses to provide a definition of \tcode{f} that makes \tcode{f} a coroutine, then the implementation of \tcode{f} will not actually be the function body that the user wrote, instead the implementation of \tcode{f} will be the compiler-generated ramp function (which is composed of different bits from the coroutine body, the promise type, etc.). Since this compiler-generated ramp function modifies the parameter object, it will notionally have a non-\tcode{const} \tcode{Widget} parameter, just like the last implementation of \tcode{f} above. However, that parameter might still be declared \tcode{const} on all declarations of \tcode{f} written by the \emph{user}, raising the question how \tcode{post} should behave if such a parameter is odr-used in its predicate. Any viable Contracts proposal needs to handle this situation in a consistent and user-friendly way.
