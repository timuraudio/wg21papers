...
To specify the point of evaluation of \tcode{pre} and \tcode{post}, we need to clarify that  operations that are specific to the compiler-generated ramp function, in particular creating a copy of the parameters and allocating the coroutine state, are considered part of the function body; and that the local variables of a coroutine body are not considered local variables of the function body, but are destroyed when the coroutine state is deallocated, which is unrelated to the point of evaluation of \tcode{post}.

Further, we need to add some clarifications around when and how the coroutine returns the object that is named by the result name in \tcode{post.}

Finally, to make it ill-formed to odr-use parameters in the predicate of \tcode{post}, we need to clarify that a coroutine effectively removes \tcode{const} from its parameter declarations. This is already implied by the existing wording for the generated ramp function body, but is currently not specified clearly enough and contains a misleading note.

The proposed wording is relative to \cite{P2900R8}.

// TODO Modify [stmt.return] ; explain relation of post with returned object AND local variables in the coroutine  // 
