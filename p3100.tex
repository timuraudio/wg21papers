...
As another example, instead of considering flowing off the end of a function ([stmt.return]/4) as a single case of UB, we could introduce separate cases for flowing off the end of an assignment operator (which has a proposed mitigation, see \cite{P2973R0}), flowing off the end of a function that returns a built-in type (which could be mitigated via returning an erroneous value, see Section~\ref{fallback}), and flowing off the end of any other non-\tcode{main} function that returns a non-\tcode{void} type.
...
\begin{figure}[b]
    ...
    \begin{tikzpicture}
        \pie[text=legend, 
            ...
            {1/Initialisation, 5/Bounds, 52/Type and Lifetime, 9/Arithmetic, 1/Threading, 1/Sequencing, 1/Assumptions, 6/Control Flow, 3/Replacement Functions, 2/Coroutines}
    \end{tikzpicture}
...
Figure \ref{piechart_categories} shows the distribution of the 81 identified cases of explicit core language UB across the 10 identified categories. 
...
For locally diagnosable cases, some kind of run-time check --- an \emph{assertion} --- could be inserted by the implementation and then evaluated at run time. The total cost of diagnosis is, therefore, equal to the cost of evaluating that check multiplied by the number of times the check needs to be evaluated.
...
% TIMUR TODO: Possibly rename this section and get rid of "fallback" terminology
...
% TIMUR TODO: MOVE THIS INTO MITIGATION SECTION
...
% TD: talk about this being only within the abstract machine. Here?
...
% TIMUR TODO: explain below that the assume semantic actually shifts the configurability over to the runtime check. But that doesn't need to be reflected in the diagram
...
% TIMUR TODO: Put this somewhere here, this is from the replacement behaviour section
%Since refined behaviour is part of the specification of correct, conforming C++ programs, it needs to be specified very precisely and applied unconditionally in order to avoid introducing language dialects. Conversely, EB only occurs in incorrect programs, and can therefore be specified more vaguely and be made configurable.
 % OLD
...
% TIMUR TODO: This isn't really accurate anymore, we only deal with a subset here, three out of six tools

% TIMUR: new structure
% Mitigation strategies - introduce the toolbox
% subsection - Not actual UB
% subsection - Make ill-formed. No case where we can do this unconditionally. But mention reinterpret_Cast in passing, defer to subsetting proposal
% subsection - Refine behaviour. One case.
...
% TIMUR TODO: Introduce the six low-level tools way earlier in the paper!
In this paper, we discuss run-time mitigation of UB via insertion of run-time checks and introduction of well-defined fallback behaviour where applicable. Thus, this paper covers two out of six low-level tools for mitigating UB in C++; see \cite{P3756R0} %TIMUR TODO: this paper does not exist, remove reference!
...
% TIMUR TODO: Remove Profiles from here

% TIMUR TODO: Add two piecharts from Sofia slides
...
% TIMUR TODO: Talk about P3541 and what SG21 decided; polls are in P2899
