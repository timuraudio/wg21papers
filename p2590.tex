...
\emph{Effects:} Implicitly creates an object \tcode{A} of type \tcode{T}, whose address is \tcode{p}, within the denoted region. Implicitly creates objects that \tcode{A} provides storage for and that reachable through a pointer value that points to \tcode{A} as described in [ptr.launder]. If \tcode{A} is a union, implicitly creates objects that are members of \tcode{A}. The object representation of \tcode{A} is the contents of the storage prior to the call to \tcode{start_lifetime_as}. The value of each created object \tcode{O} of trivially-copyable type \tcode{U} is determined as if for a call to \tcode{bit_cast<U>(O)} ([bit.cast]), except that the storage is not accessed. The value of any other created object is unspecified. \begin{note}The unspecified value may be indeterminate.\end{note}
