...
The standard library API for contract-violation handling, added to the C++26 working draft \cite{N5014} via \cite{P2900R14}, contains the type \tcode{std::contracts::contract_violation}. When the program has a user-defined contract-violation handler, and a contract violation occurs, the implementation constructs an object of this type and passes a reference to it into the contract-violation handler.

The purpose of the type \tcode{std::contracts::contract_violation} is to provide access to information about the contract violation that occurred. This includes the source location of the violated assertion and the failure mode: did the contract predicate evaluate to \tcode{false}, or did evaluation of the predicate exit via an exception? All information is provided via a set of \tcode{const} member functions.
...
The member function \mbox{\tcode{evaluation_exception()}} was intended to give the user a simple and direct way to access the exception thrown during the contract check without having to worry about any other exceptions.

\section{The problem}

When \tcode{evaluation_exception()} was proposed in \cite{P3227R1} and added to \cite{P2900R14}, there was an assumption that would  essentially be syntactic sugar for the simple logic above: check \mbox{\tcode{detection_mode()}} and invoke \mbox{\tcode{std::current_exception()}} if that mode is \mbox{\tcode{evaluation_exception}}. However, a problem arises when an exception is thrown within a contract-violation handler and \tcode{evaluation_exception()} is then invoked within the \tcode{catch} clause handling that internal exception:
...
void handle_contract_violation (const contract_violation& cv) {
  ...
  try {
    ...
    throw X;
      ...
      if (cv.detection_mode() == detection_mode::evaluation_exception) {
        // the current exception is now \tcode{X}, not whatever was thrown by the contract check!
        
        auto evaluation_exception_ptr = cv.evaluation_exception();
        // handle the original exception, not \tcode{X}
      }
  }
}
\end{codeblock}
